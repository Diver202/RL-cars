...
public class WayPointAICarController : MonoBehaviour
{
    ...
    void Update()
    {
      ...
      if(useSounds && tireScreechSound!= null){
        if(isDrifting || (isTractionLocked && Mathf.Abs(carSpeed) > 12f)){
          currentScreechVolume = Mathf.Lerp(currentScreechVolume, 1f, Time.deltaTime * screechFadeSpeed);
        }

        else{
          currentScreechVolume = Mathf.Lerp(currentScreechVolume, 0f, Time.deltaTime * screechFadeSpeed);
        }

        tireScreechSound.volume = currentScreechVolume;
      }
    ...
    {
        if(trackWaypoints == null || trackWaypoints.Count == 0)
        {
            return;
        }
        float distanceToWaypoint = Vector3.Distance(transform.position, trackWaypoints[currentWaypointIndex].position);
        
        if(distanceToWaypoint < distanceThreshold)
        {
            currentWaypointIndex = (currentWaypointIndex + 1) % trackWaypoints.Count;
        }

        Vector3 localTargetPosition = transform.InverseTransformPoint(trackWaypoints[currentWaypointIndex].position);
        float targetAngle = Vector3.SignedAngle(Vector3.forward, localTargetPosition, Vector3.up);
        // if(Mathf.Abs(targetAngle) > 60f)
        // {
        //     Brakes();
        // }
        // else
        ...
        float steeringDeadzone = 4f;
        if(localTargetPosition.x > steeringDeadzone)
        {
            TurnRight();
        }
        else if(localTargetPosition.x < -steeringDeadzone)
        {
            TurnLeft();
        }
        else
